...
public class Simbicon extends java.applet.Applet 
    ...
    private float Dt = 0.00005f; // 0.0004f the best without SPD, 0.00005f by default
    ...
    //we'll use a second buffered image to save PCA poses
    BufferedImage pcaBuffer;
    
    //image count for creating video
    int imgCnt = 0;
    private NumberFormat format = new DecimalFormat("00000");
    
    //some variables used for PCA calculation
    boolean pcaFlag = false;
    int nrSample = 10000;
    int curSample = 0;
    ...
    public void init() {
        ...
        pcaBuffer = new BufferedImage(500, 500, BufferedImage.TYPE_INT_RGB);
    ...
    public void wPDtorq(float torq[], int joint, float dposn, float kp, float kd, boolean world){
        ...
        float joint_ac = bip7.ac[2 + joint];
    ...
    public void bip7WalkFsm(float torq[]){
            ...
            for (int n=0; n<7; n++) {         // compute target angles for each joint
                    float target  = s.th[n] + Md*s.thd[n] + Mdd*s.thdd[n] + Mgnd*s.thgnd[n];         // target state + fb actions
    ...
    public void update(Graphics g){
        ...
        if(recFlag) {
	        ...
	        try {
	            if ( ! ImageIO.write( tempBuffer, "png", file) ) {
    ...
    /*
     * Method to compute PCA of a motion
     * the eigenvalues and eigenvectors will be saved to a file
     * 
     */
    public PCA computePCA() {
    		PCA pca = new PCA(data);
    		pca.center();
    		pca.covariance();
    		pca.eigen();
    		pca.print(); // print pca info to file
    		return pca;
    }
    ...
    public void drawPCA(PCA pca) {
            ...
            pcaGnd.draw(g2,m);
            File file = new File( "pca/image_eigen_" + i + ".png" );                                             
	        file = new File(file.getAbsolutePath().trim()); 
	        try {
    ...
    public void keyPressed(KeyEvent e){
        ...
        if (e.getKeyChar() == 'h' || e.getKeyChar() == 'H'){
            con.desiredGroupNumber = 3;
        }
        
        if (e.getKeyChar() == 'f' || e.getKeyChar() == 'F'){
            con.desiredGroupNumber = 4;
        }
